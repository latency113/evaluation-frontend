    ...
    >
      {!previewImage ? (
        <div className="flex-1 border-2 border-dashed border-slate-200 rounded-xl flex flex-col items-center justify-center bg-slate-50/30 relative group hover:border-blue-300 transition-all cursor-pointer min-h-[400px]">
          ...
          <span className="text-xl font-bold text-slate-800 tracking-tight">Select Schedule File</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
          ...
          <div className="lg:col-span-3 flex flex-col overflow-hidden">
            <div className="bg-slate-900 text-white p-5 rounded-lg mb-6 flex items-center justify-between shadow-sm">
              <div>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">AI Detection Status</p>
                <div className="font-bold text-lg flex items-center leading-none">
              ...
              {!isScanning && (
                <button onClick={() => setPreviewImage(null)} className="px-4 py-2 bg-white/10 hover:bg-white/20 rounded-md text-[11px] font-bold uppercase tracking-wider transition-all border border-white/10">Change File</button>
              )}
            </div>
            ...
            <div className="flex-1 overflow-auto space-y-3 pr-2 max-h-[400px]">
              {scanResult.map((item, idx) => (
                <div key={idx} className="p-5 bg-white border border-slate-200 rounded-lg shadow-sm hover:border-slate-300 transition-all group relative">
                  ...
                  <div className="space-y-3">
                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider flex items-center"><Type className="h-3 w-3 mr-1.5 text-slate-300" /> Code & Subject Name</label>
                    <div className="flex gap-3">
                      <input className="bg-slate-50 border border-slate-200 rounded-md px-3 py-2 text-xs font-bold text-blue-600 w-28 outline-none focus:border-blue-400 transition-all" value={item.code} onChange={(e) => updateScanItem(idx, "code", e.target.value)} />
                      <input className="bg-slate-50 border border-slate-200 rounded-md px-3 py-2 text-xs font-semibold text-slate-800 flex-1 outline-none focus:border-blue-400 transition-all" value={item.name} onChange={(e) => updateScanItem(idx, "name", e.target.value)} />
                    </div>
                  </div>
                </div>
              ))}
              {!isScanning && scanResult.length === 0 && (
                <div className="py-20 text-center bg-white rounded-lg border border-dashed border-slate-200">
                  ...
                  <p className="text-slate-400 font-bold uppercase tracking-wider text-[10px]">No subjects detected in this file</p>
                </div>
              )}
            </div>
            ...
            {!isScanning && scanResult.length > 0 && (
              <div className="pt-6">
                <button onClick={handleSave} className="w-full bg-blue-600 hover:bg-blue-700 text-white py-4 rounded-lg font-bold text-sm transition-all active:scale-[0.98] shadow-md uppercase tracking-wider flex items-center justify-center">
