...
export function Sidebar({ user, logout, isOpen, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed inset-y-0 left-0 w-72 bg-slate-900 text-slate-300 border-r border-slate-800 z-50 overflow-hidden flex flex-col transition-transform duration-300 ease-in-out lg:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'
          }`}
      >
        <div className="p-6 pb-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg tracking-tight text-white leading-none">
                แอดมิน
              </h1>
              <p className="text-[10px] font-medium uppercase tracking-wider text-slate-500 mt-1">
                จัดการข้อมูลผลการประเมิน
              </p>
            </div>
          </div>
          <button
            onClick={onClose}
            className="lg:hidden p-2 text-slate-500 hover:text-white hover:bg-slate-800 rounded-md transition-colors"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <div className="p-4 border-t border-slate-800">
          ...
          <button
            onClick={logout}
            className="flex items-center justify-center w-full px-4 py-2 text-xs text-slate-400 hover:text-white hover:bg-red-500/10 rounded-md transition-all group"
          >
