...
import { LogIn, User, Hash, RefreshCw, ShieldCheck, IdCard } from 'lucide-react';
...
export default function StudentLoginPage() {
  ...
  const onSubmit = async (data: LoginFormValues) => {
    ...
    try {
      ...
      const response = await studentService.getAllStudents(1, 10, data.student_code);
      const students = response.data || [];
      
      const student = students.find(
        (s: any) =>
          s.student_code === data.student_code &&
          cleanName(`${s.first_name}${s.last_name}`) === cleanName(data.full_name)
      );
      ...
      if (student) {
        ...
        setError('ไม่พบรหัสนักเรียน หรือชื่อ-นามสกุลไม่ตรงกับในระบบ กรุณาตรวจสอบอีกครั้ง');
      }
    } catch (err) {
    ...
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-[#f8fafc]">
      ...
      <div className="relative w-full max-w-[440px] px-4">
        <div className="mb-8 text-center">
          ...
          <h1 className="bg-gradient-to-r from-blue-700 to-indigo-700 bg-clip-text text-4xl font-black tracking-tight text-transparent">
            ระบบประเมินครู
          </h1>
        ...
        <div className=" border border-white bg-white/80 p-8 shadow-[0_20px_50px_rgba(8,_112,_184,_0.07)] backdrop-blur-xl md:p-10">
          {error && (
            <div className="mb-6 flex items-start gap-3 rounded-sm bg-red-50 p-4 border border-red-100">
              ...
              <p className="text-sm font-bold leading-tight text-red-600">{error}</p>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            <div className="space-y-2">
              ...
              <div className="group relative">
                ...
                <input
                  ...
                  type="text"
                  className="block w-full rounded-sm border-2 border-gray-100 bg-gray-50/50 py-4 pl-14 pr-4 font-bold text-gray-900 outline-none transition-all focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-50"
                  placeholder="รหัสประจำตัวนักเรียน นักศึกษา..."
                />
              </div>
              {errors.student_code && (
                <p className="ml-1 text-[11px] font-bold text-red-500">{errors.student_code.message}</p>
              )}
            </div>
            ...
            <div className="space-y-2">
              ...
              {errors.full_name && (
                <p className="ml-1 text-[11px] font-bold text-red-500">{errors.full_name.message}</p>
              )}
            </div>
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="group relative w-full overflow-hidden rounded-sm bg-blue-600 py-5 font-black text-lg text-white shadow-xl shadow-blue-200 transition-all hover:bg-blue-700 active:scale-[0.98] disabled:bg-blue-300"
            >
          ...
          <div className="mt-10 border-t border-gray-100 pt-8 text-center">
            <button
              ...
              className="flex items-center justify-center gap-2 mx-auto text-[11px] font-black uppercase tracking-widest text-gray-400 hover:text-blue-600 transition-colors"
            >
...
function XIcon(props: any) {
  return (<svg {...props} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>);
}
